...
class ZapierService:
    ...
    def zapier_api_check() -> str:
        return json.dumps(ZapierService.call_api_url(url=ZapierService.base_url + "/api/v1/check/", method="GET", headers=ZapierService._format_headers(os.getenv('ZAPIER_API_KEY'))))
    
    @staticmethod
    def zapier_api_get_configuration_link() -> str:
        return json.dumps(ZapierService.call_api_url(url=ZapierService.base_url + "/api/v1/configuration-link/", method="GET", headers=ZapierService._format_headers(os.getenv('ZAPIER_API_KEY'))))
    
    @staticmethod
    def zapier_api_list_exposed_actions() -> str:
        return json.dumps(ZapierService.call_api_url(url=ZapierService.base_url + "/api/v1/exposed/", method="GET", headers=ZapierService._format_headers(os.getenv('ZAPIER_API_KEY'))))
    ...
    def zapier_api_execute_action(exposed_app_action_id: str, action_parameters: str, preview_only: bool = False) -> str:
        ...
        return json.dumps(ZapierService.call_api_url(url=ZapierService.base_url + f"/api/v1/exposed/{exposed_app_action_id}/execute/", method="POST", body=body, headers=ZapierService._format_headers(os.getenv('ZAPIER_API_KEY'))))
    
    @staticmethod
    def zapier_api_execute_log(execution_log_id: str) -> str:
        return json.dumps(ZapierService.call_api_url(url=ZapierService.base_url + f"/api/v1/execution-log/{execution_log_id}/", method="GET", headers=ZapierService._format_headers(os.getenv('ZAPIER_API_KEY'))))
