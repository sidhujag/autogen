...
class CodeRepositoryService:
    @staticmethod
    async def get_code_repository(coding_repository_model):
        ...
        if code_repository is None:
            backend_code_repositories, err = await BackendService.get_backend_code_repositories([coding_repository_model])
            if err is None and backend_code_repositories:
                ...
                if err is None and code_repository:
                    ...
                    await BackendService.delete_backend_code_repositories([DeleteCodeRepositoryModel(name=coding_repository_model.name)])
        return code_repository
    
    @staticmethod
    async def get_code_repository_info(repository_name: str) -> str:
        from . import MakeService, CodeRepositoryInfo, GetCodeRepositoryModel
        backend_code_repository = await CodeRepositoryService.get_code_repository(GetCodeRepositoryModel(name=repository_name))
    ...
    @staticmethod
    async def discover_code_repositories(query: str) -> str:
        from . import BackendService, DiscoverCodeRepositoryModel
        response, err = await BackendService.discover_backend_code_repositories(DiscoverCodeRepositoryModel(query=query))
        if err is not None:
            return err
        return response
    ...
    @staticmethod
    async def upsert_code_repositories(upsert_models):
        from . import BackendService, GetCodeRepositoryModel, DeleteCodeRepositoryModel
        # Step 1: Upsert all in batch
        err = await BackendService.upsert_backend_code_repositories(upsert_models)
        ...
        backend_code_repositories, err = await BackendService.get_backend_code_repositories(get_code_repository_models)
        if err:
            return None, err
        ...
        for backend_code_repository in backend_code_repositories:
            ...
            if err is not None:
                await BackendService.delete_backend_code_repositories([DeleteCodeRepositoryModel(name=backend_code_repository.name)])
                return None, err
