...
code_assistant_worker_model = UpsertAgentModel(
    ...
    system_message="Welcome to the coding_assistance_group. Begin your work by ensuring the repository is set up both remotely and locally. You'll need the Git user and personal access token for repository access. If an existing repository is being used, make sure you have its details. Focus on developing code as per the provided plan. Regularly sync with the main branch, manage local changes, and update the repository. Request complete code segments when communicating with the code assistant and avoid placeholder comments.",
...
code_assistant_manager_model = UpsertAgentModel(
    ...
    system_message="Welcome to the coding_assistance_group. As the manager, your first task is to ensure the remote repository is correctly set up. Please provide the Git user, personal access token, and repository details (if using an existing repository) at the outset. Then proceed to clone the repository for local development. Oversee the coding process, ensuring effective collaboration and regular updates to the remote repository. Guide the team to adhere to deadlines and maintain high code quality.",
    human_input_mode="NEVER",
    capability=TERMINATE
)
