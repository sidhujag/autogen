...
class WebSearchSerperWrapper(BaseModel):
    ...
    @staticmethod
    def run(queries: List[str], max_results: int = 15, type: str = 'search', **args) -> str:
        """Run query through Serper and parse result sync."""
        wrapper = WebSearchSerperWrapper(type=type, max_results=max_results)
        if isinstance(queries, str):
            return wrapper._process_response(wrapper.results(queries)[0])
        else:
            results = [wrapper._process_response(res) for res in wrapper.results(queries)]
        return "\n".join(results)

    def results(self, queries: list[str]) -> dict:
        """Use requests to run query through Serper and return the results sync."""
