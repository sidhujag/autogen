...
- [AutoBuild](/blog/2023/11/26/Agent-AutoBuild). AutoBuild is an ongoing area of research to automatically create or select a group of agents for a given task and objective. If successful, it will greatly reduce the effort from users or developers when using the multi-agent technology. It also paves the way for agentic decomposition to handle complex tasks. It is available as an experimental feature and demonstrated in two modes: free-form [creation](https://github.com/superdapp/superdappstudio/blob/0.2/notebook/autobuild_basic.ipynb) and [selection](https://github.com/superdapp/superdappstudio/blob/0.2/notebook/autobuild_agent_library.ipynb) from a library.
...
- new features like stateful code execution, [tool decorators](/docs/Use-Cases/agent_chat#tool-calling), [long context handling](https://github.com/superdapp/superdappstudio/blob/0.2/notebook/agentchat_capability_long_context_handling.ipynb), [web agents](https://github.com/superdapp/superdappstudio/blob/0.2/notebook/agentchat_surfer.ipynb).
- integration examples like using [guidance](https://github.com/superdapp/superdappstudio/blob/0.2/notebook/agentchat_guidance.ipynb) to generate structured response.
